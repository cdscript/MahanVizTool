...
# }
create.treatment.array.cust <- function(doy.range,treatment.col1,num.raw.col,treat.range,
                                        num.treatments,rawdata,temp.marker,y.temp,midnight)
{
  ...
  if(temp.marker == 1)#DOY VIEW
  {
    ...
    for(t in 1:num.raw.col)
  {
      temp.start <- 1#midnight[which(doy.range == treatment.col1)]
      temp.end <- temp.start + 95
    for(j in 1:treat.range)
    {
      if(min(doy.range) > treatment.col1)#| (j > (max(doy.range)-treatment.col1)))
      {
        treatment.array[,j,t] <- c(0)
        temp.start <- temp.end + 1
        temp.end <- temp.start + 95
      ...
      if(j == 1){
        temp.start <- 1#tod.range.start#midnight[which(doy.range == treatment.col1)] + tod.range.start
        temp.end <- temp.start + 95#tod.range.end
      } 
      treatment.array[,j,t] <- cbind(rawdata[temp.start:temp.end, y.temp])
      temp.start <- temp.start + 1
      temp.end <- temp.end + 95
      }
    }#End of for loop of treat.range     
    y.temp <- y.temp + 1
  }
  
  }#End of DOY view
  
  if(temp.marker == 2)#DAP VIEW
  {
    
    for(t in 1:num.raw.col)
    {
      temp.start <- min(which(is.na(rawdata[,(t+1)]) == FALSE))
      temp.end <- temp.start + 95
      for(j in 1:treat.range)
      {
          treatment.array[,j,t] <- cbind(rawdata[temp.start:temp.end, y.temp])
          temp.start <- temp.start + 95
          temp.end <- temp.end + 95
